...
class AnalyticsController extends Controller
{
    ...
    public function providerAnalytics()
    {
        ...
        // Ensure these queries return 0 if no data exists
        $completedServices = DB::table('service_requests')
            ->where('status', 'completed')
            ->where('provider_id', $userId)
            ->count() ?? 0;
        ...
        // Ensure default values for satisfaction data
        $customerSatisfactionData = DB::table('ratings')
            ->selectRaw('
                AVG(communication) * 10 AS communication,
                AVG(fairness) * 10 AS fairness,
        ...
        // Handle no cancellation or completion data
        $cancellationData = DB::table('service_requests')
            ->selectRaw('
                COUNT(*) AS total_requests,
                SUM(CASE WHEN status = "cancelled" THEN 1 ELSE 0 END) AS cancelled_requests
        ...
        $cancellationRate = $cancellationData->total_requests > 0 
            ? ($cancellationData->cancelled_requests / $cancellationData->total_requests * 100) 
            : 0;
        ...
        $completionRate = $completionData->total_requests > 0 
            ? ($completionData->completed_requests / $completionData->total_requests * 100) 
            : 0;
    
        // Handle if no service requests for most availed service and loyal seeker
        $mostAvailedService = DB::table('service_requests')
            ->select('category', DB::raw('COUNT(*) AS completed_services'))
            ->where('status', 'completed')
            ->where('provider_id', $userId)
            ->groupBy('category')
            ->orderBy('completed_services', 'DESC')
            ->first() ?? (object) ['category' => 'None', 'completed_services' => 0];
        ...
        $mostLoyalSeeker = DB::table('service_requests')
            ...
            ->groupBy('users.id', 'users.name')
            ->orderBy('completed_services', 'DESC')
            ->first() ?? (object) ['name' => 'None', 'completed_services' => 0];
    
        // Ensure empty collection for employees if no data
        ...
        // Handle service categories (ensure empty if no data)
