                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <div class="flex items-center justify-end w-full">
                                        @if ($serviceRequest->hasAcceptedBid())
                                            ...
                                            <a href="{{ route('channel.seeker', ['serviceRequestId' => $serviceRequest->id]) }}"
                                                class="border border-custom-lightest-blue text-custom-lightest-blue px-4 py-2 rounded-md ml-4 flex items-center hover:bg-custom-lightest-blue hover:text-white transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
   ...
   <div x-show="showBidsPanel" x-transition class="fixed inset-0 bg-gray-800 bg-opacity-50 z-50 flex justify-end p-4" style="display: none;">
    <div class="bg-white p-6 shadow-lg rounded-lg w-full max-w-lg relative" @click.stop>
        ...
        <div class="flex justify-between mb-4">
            ...
            <div>
                <button @click="sortBids('asc')" class="bg-gray-200 px-2 py-1 rounded">Lowest to Highest</button>
                <button @click="sortBids('desc')" class="bg-gray-200 px-2 py-1 rounded">Highest to Lowest</button>
            </div>
        </div>
                    ...
                    <div class="flex justify-between items-start">
                        <div class="flex-grow">
                            ...
                            <div class="flex justify-end space-x-2">
                                <a :href="`/profile/${bid.bidder.id}`" @click="console.log(bid.bidder.id)" class="bg-custom-lightest-blue text-white px-4 py-2 rounded hover:bg-blue-600">View Profile</a>
                ...
                <div class="bg-white p-4">
                    ...
                    <form action="{{ route('report.store') }}" method="POST">
                        ...
                        <textarea name="details" required
                            class="h-16 mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                ...
                return {
                    ...
                    async fetchBids(requestId) {
                        ...
                        try {
                          ...
                          async sortBids(order) {
                console.log(`Sorting bids in ${order} order for request ID: ${this.selectedRequestId}`);
                try {
                    const response = await fetch(`/api/service-requests/${this.selectedRequestId}/bids?sort=${order}`);
                    if (!response.ok) {
                        console.error(`Failed to fetch sorted bids: ${response.status} - ${response.statusText}`);
                        throw new Error('Network response was not ok');
                    }
                    const data = await response.json();
                    console.log('Fetched sorted bids:', data); // Log the sorted bids
                    this.bids = data; // Update the bids with sorted data
                } catch (error) {
                    console.error('There was a problem with the fetch operation:', error);
                }
            },
