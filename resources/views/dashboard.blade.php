    ...
    <div x-data="dashboard()" class="py-12">
                            ...
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    <x-outline-button href="{{ route('service-requests.edit', $serviceRequest) }}"
                                        class="flex-1 md:flex-none w-full md:w-auto border-gray-500 hover:bg-gray-500 hover:border-gray-500">
                                ...
                                <div class="flex items-center">
                                    @if ($serviceRequest->hasAcceptedBid())
                                        ...
                                        @if (!$serviceRequest->isCompleted())
                                            <!-- Assuming isCompleted() is a method or attribute indicating completion -->
                                            <a href="{{ route('channel.seeker', ['serviceRequest' => $serviceRequest->id]) }}"
                                                class="text-blue-500 underline ml-4">View Channel</a>
                                        @else
        ...
        <div x-show="showBidsPanel" class="fixed inset-0 bg-gray-800 bg-opacity-50 z-50 flex justify-end p-4">
            <div class="bg-white p-6 shadow-lg rounded-lg w-full max-w-lg relative" @click.stop>
                <div class="pb-8">
                    <button @click="closeBidsPanel()"
                        class="absolute top-4 right-4 text-red-500 text-xl">&times;</button>
                </div>
                                ...
                                <div class="flex-grow">
                                    <div class="flex justify-between items-center mb-2">
                                        ...
                                        <div class="text-gray-600 ml-2"
                                            x-text="new Date(bid.created_at).toLocaleString()"></div>
                                    </div>
                                    ...
                                    <div class="flex justify-end space-x-2">
                                        <button @click="viewProfile(bid.bidder.id)"
                                            class="bg-custom-lightest-blue text-white px-4 py-2 rounded hover:bg-blue-600">View
                                            Profile</button>
                                        <button x-show="!bid.confirmed" @click="confirmBid(bid.id, selectedRequestId)"
                                            class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 mr-2">Accept
                                            Bid</button>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="col-span-1">
                        <div class="flex items-center justify-between mb-2">
                            <div class="font-semibold text-xl text-gray-700"
                                x-text="profile.providerDetails.serviceCategory"></div>
                            <div class="font-semibold text-xl text-gray-700"
                                x-text="profile.providerDetails.years_of_experience + ' years of experience'"></div>
                        </div>
                        <div class="flex justify-between">
                            <div class="text-gray-600 mb-4" x-text="profile.providerDetails.description"></div>
                            <div class="text-gray-600 mb-2"
                                x-text="'Have Tools: ' + (profile.providerDetails.have_tools ? 'Yes' : 'No')"></div>
                        </div>
                        <div class="flex items-center mb-2">
                            <!-- Email Icon and Text Alignment -->
                            <span class="material-symbols-outlined text-gray-600 mr-2">mail</span>
                            <span class="text-gray-600">Email: <span
                                    x-text="profile.providerDetails.work_email"></span></span>
                        </div>
                        <div class="flex items-center mb-2">
                            <!-- Phone Icon and Text Alignment -->
                            <span class="material-symbols-outlined text-gray-600 mr-2">call</span>
                            <span class="text-gray-600">Phone: <span
                                    x-text="profile.providerDetails.contact_number"></span></span>
                        </div>
                    </div>
                    <div class="mt-6 flex justify-end space-x-4">
                        <button @click="confirmBid(profile.bidId, selectedRequestId)"
                            class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">Confirm Bid</button>
