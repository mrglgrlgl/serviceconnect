                ...
                @if (isset(Auth::user()->philID) && Auth::user()->philID->status === 'Accepted')
                    ...
                    @else
                        @foreach ($serviceRequests as $serviceRequest)
                            @if ($serviceRequest->provider_id == Auth::user()->id || !$serviceRequest->is_direct_hire)
                                <div class="p-4 border border-gray-300 shadow-sm rounded-lg md:mb-4 {{ $serviceRequest->is_direct_hire ? 'bg-yellow-100' : '' }}">
                                    ...
                                    <div class="flex justify-end items-center space-x-2 mt-4">
                                        ...
                                        <div x-data="{ showModal: false, showConfirmationModal: false }">
                                            ...
                                            @if ($serviceRequest->status == 'completed' && $serviceRequest->provider_id == Auth::user()->id)
                                                @if ($reportExists)
