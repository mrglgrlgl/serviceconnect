        ...
        Filter
            ...
            <div class="text-right mb-4">
                <a href="{{ route('agency.employees.create') }}"
                    class="bg-custom-agency-secondary text-white py-3 px-6 rounded-md font-semibold shadow-md duration-300 hover:bg-gray-700 hover:shadow-lg transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
                    <span class="material-icons align-middle">add_circle</span> Add New Employee
                </a>
            </div>

            @if ($employees->isEmpty())
                <div class="bg-red-100 text-red-800 p-4 rounded-lg text-center font-medium mb-8">
                    <strong>No employees found.</strong> Please add employee.
                </div>
            @else
                                ...
                                <tr class="border-t border-gray-200 hover:bg-gray-100 transition ease-in-out duration-150 text-md">
                                    <td class="p-4 text-center">
                                        @if ($employee->photo)
                                            <img src="{{ asset('storage/' . $employee->photo) }}"
                                                alt="{{ $employee->name }}" class="w-16 h-16 rounded-md shadow-sm object-cover">
                                        @else
                                    ...
                                    <td class="p-4 text-center">
                                        <div class="flex justify-center space-x-2">
                                            <!-- View Button -->
                                            <a href="{{ route('agency.employees.show', $employee->id) }}" class="border-2 border-blue-600 text-blue-600 py-2 px-4 rounded-md font-semibold shadow-sm hover:bg-blue-600 hover:text-white hover:shadow-lg transition ease-in-out duration-300 transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
                                            ...
                                            <form action="{{ route('agency.employees.destroy', $employee->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this employee?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="bg-red-500 border-2 border-red-500 text-white py-2 px-4 rounded-md font-semibold shadow-md hover:bg-red-600 hover:shadow-lg  ease-in-out duration-300 transition-transform hover:scale-105">
