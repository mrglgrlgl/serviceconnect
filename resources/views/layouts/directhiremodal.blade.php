    ...
    <div class="mt-6 px-6 py-8 bg-white shadow-md sm:rounded-lg w-full md:w-5/12 mx-auto">
        <div class="flex flex-col items-center">
            <div class="text-2xl font-bold text-custom-light-blue">
                {{ __('Direct Hire Service Request') }}
            </div>

            <div class="w-full md:w-7/12 space-y-6 mx-auto mt-4">
                <form id="directHireForm" action="{{ route('direct-hire.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="provider_id" value="{{ $provider->id }}">
                    <input type="hidden" name="category" value="{{ $provider->providerDetails->serviceCategory }}">

                    <div class="border-t my-4 w-full pb-6"></div>

                    <div class="mb-4">
                        <label for="category" class="block text-base text-custom-default-text">Category</label>
                        <x-text-input type="text" id="category" name="category" value="{{ $provider->providerDetails->serviceCategory }}" readonly class="mt-1 block w-full sm:text-sm rounded-md bg-gray-100" />
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="flex items-center">
                            ...
                            <span class="text-sm text-gray-700">By creating the service request I agree to the <a href="/terms" target="_blank" class="text-blue-500 underline">terms and conditions</a>.</span>
                        </label>
                    </div>
