                ...
                @if(isset($certifications))
                    @foreach ($certifications as $certification)
                        <div class="border border-gray-300 p-4 rounded-md flex justify-between">
                            <div>
                                <strong class="text-gray-800">{{ $certification->name }}</strong> - {{ $certification->issuing_organization }}
                            </div>
                            @if ($certification->file_path)
                                <a href="{{ Storage::url($certification->file_path) }}" target="_blank" class="text-blue-500 ml-2 underline">View Certificate</a>
                            @endif
                        </div>
                    @endforeach
                @else
